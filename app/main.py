...
def decide_packages(packages: list[dict]) -> dict:
    ...
    body = {
        ...
        # NOTE: gpt-5-nano (and other reasoning-tier models on AI Pipe) reject
        # any explicit "temperature" other than the default of 1 with a 400
        # "unsupported_value" error. Do not set temperature here.
    }
    headers = {
    ...
    for attempt in range(2):  # one retry with a repair nudge
        ...
        try:
            ...
            if "choices" not in data or not data["choices"]:
                raise ValueError(f"unexpected AI Pipe response shape (no choices): {data!r}")
